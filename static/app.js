...
async function loadStockChart(symbol, timeframe) {
  ...
  try {
    ...
    // some mobile browsers/slow Render cold starts.
    requestAnimationFrame(() => renderStockChart(d));
  } catch (error) {
...
window.addEventListener("resize", () => {
  ...
  pairs.forEach(([chart, id]) => {
    const el = $(id);
    if (chart && el) {
      try { chart.applyOptions({ width: Math.max(300, el.clientWidth || 300) }); } catch (e) {}
    }
  });
});
